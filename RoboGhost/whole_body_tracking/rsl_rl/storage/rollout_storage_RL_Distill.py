...
class RolloutStorage:
    ...
    def __init__(
        ...
        if training_type == "rl":
            ...
            self.actions_teacher = torch.zeros(num_transitions_per_env, num_envs, *actions_shape, device=self.device)
            self.teacher_obs = torch.zeros(num_transitions_per_env, num_envs, *teacher_obs_shape, device=self.device)
    ...
    def add_transitions(self, transition: Transition):
        ...
        if self.if_distill:
            self.actions_teacher[self.step].copy_(transition.actions_teacher)
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        #把rollout的数据flatten，self.actions.shape = [24, num_envs, 29] -> actions.shape = [num_envs*24, 29]
    ...
    def mini_batch_generator_rl_distill(self, num_mini_batches, num_epochs=8):
        if self.training_type != "rl":
            raise ValueError("This function is only available for reinforcement learning training.")
        batch_size = self.num_envs * self.num_transitions_per_env
        mini_batch_size = batch_size // num_mini_batches
        ...
        teacher_obs = self.teacher_obs.flatten(0,1)
        # For RND
        if self.rnd_state_shape is not None:
            rnd_state = self.rnd_state.flatten(0, 1)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                teacher_obs_batch = teacher_obs[batch_idx]
                privileged_observations_batch = privileged_observations[batch_idx]
                actions_batch = actions[batch_idx]
                ...
                yield obs_batch, privileged_observations_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (
                    None,
                    None,
                ), None, rnd_state_batch, actions_teacher_batch, teacher_obs_batch
