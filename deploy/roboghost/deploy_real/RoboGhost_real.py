...
class Controller:
    def __init__(self, motion_id, motion_latent_path, config: Config) -> None:
        self.config = config
        self.remote_controller = RemoteController()
        self.load_motion_latent = get_motion_latent(motion_latent_path)
        self.motion_id = torch.tensor([motion_id], device=self.load_motion_latent._device)
    ...
    def run(self):
        ...
        motion_latent = self.load_motion_latent.get_motion_latent(self.motion_id, self.timestep)
        qpos_urdf = qj_obs
        qj_obs_seq =  np.array([qpos_urdf[joint_xml.index(joint)] for joint in joint_seq])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--net", type=str, required=True, help="network interface")
    parser.add_argument("--config", type=str, required=True, help="path to config file")
    parser.add_argument("--motion_id", type=int, default=0, help="motion id to track")
    ...
    motion_latent_path=args.motion_latent_path, 
    config=config
    )
